...
use GeTui\protobuf\type\PBScalar;
use GeTui\protobuf\type\PBEnum;
use GeTui\protobuf\type\PBBytes;
use GeTui\protobuf\type\PBString;
use GeTui\protobuf\type\PBInt;
use GeTui\protobuf\type\PBBool;
use GeTui\protobuf\type\PBSignedInt;
use GeTui\protobuf\reader\PBInputReader;
...
abstract class PBMessage
{
    ...
    {
        $className = $this->getOriginClassName($this->fields[$index]);
        return $this->values[$index][] = new $className();
    }
    ...
    protected function _set_value($index, $value)
    {
        ...
        {
            $name = $this->getOriginClassName($this->fields[$index]);
            $this->values[$index] = new $name();
